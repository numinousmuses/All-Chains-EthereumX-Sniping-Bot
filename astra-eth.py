...
def BY():
	...
	A_[AR]='https://main-light.eth.linkpool.io/';A(f,BW,A_)
def BZ():
	def A(path2,file_name,data2):
		A=r+path2+A4+file_name
		with L(A,A5)as B:e.dump(data2,B,indent=2)
...
A9=C.toChecksumAddress('0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2')
g=C.toChecksumAddress('0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48')
U=d(L(BM+'erc20.abi'))
V=C.eth.contract(address=R.toChecksumAddress('0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D'),abi=d(L(BM+'router.abi')))
BC=C.eth.contract(address=R.toChecksumAddress('0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f'),abi=d(L(BM+'factory.abi')))
...
def Bp():
	...
	while Q:
		try:
			N=M.functions.balanceOf(G).call()-1;F=I(C.fromWei(V.functions.getAmountsOut(N,[H,A9]).call()[-1],c));D=Ap(I(F)/I(q)*100,5);A('ETH Value Now: {} / '.format(Cg%F)+Ch.format(D)+Ci+A3(B)+'%')
			if E!=0:
